...
class Day12 : Day {
    ...
    override fun executePart2(): Long {
        val nodes = HashSet<Node>()
        File("src/main/resources/day12-2.txt").readLines().forEach { s ->
            val parts = s.split('-')
            val l = parts[0]
            val r = parts[1]
        ...
        fun traverse(node: Node, path: List<Node>) {
            if (node.isStart() && path.any { it.id == node.id }) {
                return
            }
            ...
            if (node.isSmall() && (path.groupingBy { it }.eachCount()
                    .any { entry -> entry.value == 2 && entry.key.isSmall() }) && path.contains(node)
            ) {
                return
            }
